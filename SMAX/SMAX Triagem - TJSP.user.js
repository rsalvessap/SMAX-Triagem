...
// @noframes
// @downloadURL  https://github.com/rsalvessap/SMAX-Triagem/raw/refs/heads/master/SMAX/SMAX%20Triagem%20-%20TJSP.user.js
// @updateURL    https://github.com/rsalvessap/SMAX-Triagem/raw/refs/heads/master/SMAX/SMAX%20Triagem%20-%20TJSP.user.js
...
`);
  ...
  const SettingsPanel = (() => {
    ...
    // Equipes são somente-leitura na versão regular (gerenciadas pelo ADM)
    ...
    const renderTeamsList = () => {
      ...
      const listHtml = allTeams.map(t => {
        ...
        return `
          <div class="smax-team-item" style="border:1px solid var(--sp-border);border-radius:10px;padding:10px 12px;margin-bottom:8px;background:var(--sp-surface-2);transition:border-color .15s ease;">
            <div style="display:flex;justify-content:space-between;align-items:center;">
              ...
              <span style="font-size:11px;color:var(--sp-text-muted);padding:6px 8px;">somente leitura</span>
            </div>
          </div>
        `;
      }).join('');
      ...
      return `
        <div style="margin-top:16px;border-top:1px solid var(--sp-border);padding-top:12px;">
          ...
          <div class="smax-sp-muted" style="margin-bottom:10px;">🔒 Gerenciado pela versão ADM. Equipes são sincronizadas automaticamente via configuração compartilhada.</div>
    ...
    /* publishConfigToGit: disponível apenas na versão ADM */
    ...
    const renderSectionGeral = () => {
      ...
      if (importLog.length) {
        importDetailsHtml = '<div style="margin-top:10px;border:1px solid var(--sp-border);border-radius:8px;overflow:hidden;">' +
      ...
      return `
        ...
        <div class="smax-sp-card">
          ...
          <div style="border-top:1px solid var(--sp-border);margin-top:14px;padding-top:14px;">
            <div class="smax-sp-muted">🔒 A publicação para a equipe (Git) está disponível apenas na versão ADM.</div>
          </div>
        </div>`;
    };
    ...
    const wireGeralEvents = () => {
      ...
      if (cfgTextarea) cfgTextarea.value = JSON.stringify({ _info: 'Cole aqui o JSON de configuração compartilhada para importar.' }, null, 2);
  ...
  const SharedConfig = (() => {
    ...
    return { init, refresh, get, getStatus, onTeamsLoaded, getImportLog };
  })();
